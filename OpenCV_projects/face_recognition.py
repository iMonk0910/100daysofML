...
while True:
   ...
   ## Img = imutils.resize(Img, width=500) #resize
    ...
    faces = haar_cascade.detectMultiScale(grayImg,1.3, 4) #gets cordinates of the face
    ...
    for (x, y, w, h) in faces:
        ...
        if prediction[1]<800:
          cv2.putText(Img, "%s - %.0f" % (names[prediction[0]],prediction[1]),(x-10, y-20))
          print(names[prediction[0]])
          cnt = 0
        else:
          cnt+=1
          cv2.putText(Img,'Unknown'(x-10, y-20), cv2.FONT_HERSHEY_SIMPLEX, 1,(0, 255, 0), 2)      
          if(cnt>100):
